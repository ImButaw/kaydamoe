...
def show_lan_choice():
    ...
    def do_discover():
        status_var.set("Scanning LAN for host...")
        dialog.update()

        def run():
            try:
                found_ip = api_client.discover_host(timeout=1.0, attempts=4)
                if not found_ip:
                    raise RuntimeError("No host found on LAN.")
                dialog.after(0, lambda: ip_var.set(found_ip))
                dialog.after(0, lambda: status_var.set(f"Host found: {found_ip}"))
            except Exception as e:
                dialog.after(0, lambda: status_var.set(f"Discovery failed: {e}"))
    ...
    CTkButton(join_card, text="AUTO-DISCOVER HOST", height=38,
              fg_color=SURFACE2, hover_color=SURFACE, text_color=GREEN,
              border_width=1, border_color=GREEN,
              font=ctk.CTkFont(weight="bold"), corner_radius=8, command=do_discover).grid(
        row=3, column=0, sticky="ew", padx=20, pady=(0, 8))
    ...
    CTkButton(join_card, text="CONNECT & JOIN", height=40,
        ...
        row=4, column=0, sticky="ew", padx=20, pady=(0, 16))
