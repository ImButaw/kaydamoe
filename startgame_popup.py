...
def open_start_game(parent, on_start):
    ...
    def start_clicked():
        ...
        try:
            ...
            err_txt = str(e).lower()
            if "game label already exists" in err_txt or "unique constraint" in err_txt:
                msg_var.set("Game label already exists. Pick a unique label.")
                game_var.set(api_client.get_next_label())
            else:
